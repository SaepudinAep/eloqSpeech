// eloq_acoustic_engine.js - V1.4 (Final Hybrid: Oscilloscope + Consistency & Fatigue)
// Features: Multi-Trial, Consistency Index (CV), Fatigue Index, Dynamic Hue Shift, SPA Safe
...
const CONFIG = {
    ...
    COLORS: {
        ...
        TRIAL_1: { solid: '#3b82f6', bg: 'rgba(59, 130, 246, 0.2)' }, // Blue
        TRIAL_2: { solid: '#a855f7', bg: 'rgba(168, 85, 247, 0.2)' }, // Purple
        TRIAL_3: { solid: '#ef4444', bg: 'rgba(239, 68, 68, 0.2)' }   // Red
    }
};
...
const STYLES = `
    * { box-sizing: border-box; }
    ...
    .eae-title { font-weight: 700; color: #1e293b; font-size: 1.1rem; }
    .eae-instruction { font-size: 0.9rem; color: #64748b; font-weight: 500; }
...
let state = {
    ...
    trialData: { volumeLog: [], startTime: 0, lastVocalTime: 0, mptDuration: 0, hasStarted: false },
...
function nukeArtifacts() {
    ...
    if (state.sessionLogs && state.sessionLogs.length > 0) {
        state.sessionLogs.forEach(log => {
            if (log.audioUrl) URL.revokeObjectURL(log.audioUrl);
        });
    }
...
function showStartScreen(appRoot) {
    ...
    overlay.innerHTML = `
        <div class="eae-menu-container" style="margin-top:auto; margin-bottom:auto;">
            ...
            <button class="eae-btn" id="btn-mode-a">
                <span>🗣️ Resonansi Pita Suara</span>
                <span style="font-size:0.8rem; font-weight:normal; color:#64748b;">Target: "A", "I", "U"</span>
            </button>
            <button class="eae-btn" id="btn-mode-s">
                <span>🌬️ Aliran Udara / Frikasi</span>
                <span style="font-size:0.8rem; font-weight:normal; color:#64748b;">Target: "Sssss"</span>
            </button>
            <button class="eae-btn" id="btn-mode-z">
                <span>🐝 Resonansi + Frikasi</span>
                <span style="font-size:0.8rem; font-weight:normal; color:#64748b;">Target: "Zzzzz"</span>
            </button>
        </div>
    `;
...
function startCalibration() {
    ...
    const ws = document.getElementById('eae-workspace');
    ws.innerHTML = `
        <div class="eae-orb-container"><div class="eae-orb-base" style="background:#f59e0b;">🤫</div></div>
        <div class="eae-status-text">Harap Hening...<br><span style="font-size:0.9rem; font-weight:normal;">Mengukur ambang batas bising ruangan (Noise Floor).</span></div>
    `;
...
function prepareNextTrial() {
    ...
    ws.innerHTML = `
        ...
        <div class="eae-orb-container">
            <div class="eae-orb-active" id="eae-orb-visual" style="background:${themeBg}"></div>
            <div class="eae-orb-base" style="background:${themeColor};">${displayIcon}</div>
        </div>
        <div class="eae-status-text" id="eae-live-status">Uji ${state.currentTrial} dari ${CONFIG.AUDIO.MAX_TRIALS}.<br>Tarik napas dan mulai bunyikan <b>${state.targetLabel}</b>.</div>
        <button class="eae-btn-danger" id="btn-force-stop" style="display:none; margin-top:20px;">⏹ Hentikan Perekaman</button>
    `;
    ...
    state.trialData = { volumeLog: [], startTime: 0, lastVocalTime: 0, mptDuration: 0, hasStarted: false };
    state.audioChunks = [];
...
function monitorLiveAudio() {
    ...
    if (!state.trialData.hasStarted) {
        if (isVocalizing) {
            state.trialData.hasStarted = true;
            state.trialData.startTime = now;
            state.trialData.lastVocalTime = now;
            document.getElementById('eae-live-status').innerHTML = '<span style="color:#10b981;">Sedang merekam suara...</span>';
            document.getElementById('btn-force-stop').style.display = 'block';
        }
    } else {
        ...
        if (isVocalizing) {
            state.trialData.lastVocalTime = now;
            state.trialData.mptDuration = dur;
        } else {
            if (now - state.trialData.lastVocalTime > CONFIG.AUDIO.SILENCE_TOLERANCE_MS) {
                stopTrialRecording();
                return;
            }
        }
...
function processTrialAudio() {
    ...
    state.sessionLogs.push({
        ...
        mpt: state.trialData.mptDuration,
        avgIntensity: avgVol,
        shimmer: shimmer,
        volumeData: [...vols],
        audioUrl: url
    ...
    if (state.currentTrial < CONFIG.AUDIO.MAX_TRIALS) {
        state.currentTrial++;
        setTimeout(() => { if(state.isActive) prepareNextTrial(); }, 1500);
    } else {
        showDashboard();
    }
}

// --- PHASE 4: DASHBOARD, CONSISTENCY & FATIGUE ---
function showDashboard() {
    const logs = state.sessionLogs;
    const mpts = logs.map(l => l.mpt);
    
    const maxMpt = Math.max(...mpts);
    const avgInt = logs.reduce((s,l)=>s+l.avgIntensity,0) / logs.length;
    const avgShim = logs.reduce((s,l)=>s+l.shimmer,0) / logs.length;
    
    // 1. Fatigue Index (Kelelahan): Anjlokan dari Uji 1 ke Uji 3
    let fatigue = 0;
    if (logs[0].mpt > 0) {
    ...
    // 2. Consistency Index (Konsistensi): Coefficient of Variation (CV)
    let consistency = 0;
    const meanMpt = mpts.reduce((a,b)=>a+b,0) / mpts.length;
    if (meanMpt > 0) {
        const variance = mpts.reduce((s, val) => s + Math.pow(val - meanMpt, 2), 0) / mpts.length;
        const stdDev = Math.sqrt(variance);
        const cv = (stdDev / meanMpt) * 100;
    ...
    logs.forEach((log, index) => {
        ...
        cardsHtml += `
            <div class="eae-card">
                <div class="eae-card-header">
                    <div class="eae-card-title" style="color:${tColor};">Uji ${log.trial}</div>
                    <div style="font-weight:800; font-size:1.2rem; color:#1e293b;">${log.mpt.toFixed(2)}s</div>
                </div>
                <div class="eae-stats-row">
                    <div class="eae-stat-item">Vol: <span class="eae-s-val">${log.avgIntensity.toFixed(1)}</span></div>
                    <div class="eae-stat-item">Tremor: <span class="eae-s-val">${log.shimmer.toFixed(1)}%</span></div>
                </div>
                <div class="eae-wave-container">
                    <canvas id="eae-cvs-${index}" class="eae-wave-canvas"></canvas>
                    <div id="eae-ph-${index}" class="eae-playhead"></div>
                </div>
                <div>
                    <audio id="eae-aud-${index}" src="${log.audioUrl}"></audio>
                    <button class="eae-control-btn" id="btn-play-${index}">▶️ Putar Uji ${log.trial}</button>
                </div>
            </div>
        `;
    });
    ...
    overlay.innerHTML = `
        <div class="eae-dash-wrapper">
            ...
            <div class="eae-summary-box">
                <div class="eae-metric-col">
                    <div class="eae-m-val" style="color:#3b82f6;">${maxMpt.toFixed(2)}s</div>
                    <div class="eae-m-lbl">Max MPT</div>
                </div>
                <div class="eae-metric-col">
                    <div class="eae-m-val" style="color:${shimColor};">${avgShim.toFixed(1)}%</div>
                    <div class="eae-m-lbl">Indeks Tremor</div>
                </div>
                <div class="eae-metric-col">
                    <div class="eae-m-val" style="color:${fatigueColor};">${fatigue.toFixed(1)}%</div>
                    <div class="eae-m-lbl">Kelelahan (Fatigue)</div>
                </div>
                <div class="eae-metric-col">
                    <div class="eae-m-val" style="color:${consColor};">${consistency.toFixed(1)}%</div>
                    <div class="eae-m-lbl">Konsistensi</div>
                </div>
            </div>
            
            <div class="eae-dash-grid">
                ${cardsHtml}
            </div>
        </div>
    `;
    ...
    setTimeout(() => {
        logs.forEach((log, index) => {
            drawOscilloscopeDashboard(`eae-cvs-${index}`, log.volumeData, index);
            setupPlaybackListener(index);
        });
    }, 150);
}
...
function drawOscilloscopeDashboard(canvasId, volumeData, trialIndex) {
    ...
    const step = w / len;
    for (let i = 0; i < len; i++) {
        ctx.lineTo(i * step, h - (volumeData[i] / maxVol) * h);
    }
...
function setupPlaybackListener(index) {
    ...
    const audio = document.getElementById(`eae-aud-${index}`);
    const playhead = document.getElementById(`eae-ph-${index}`);
    const cvs = document.getElementById(`eae-cvs-${index}`);
    ...
    btn.onclick = () => {
        if (audio.paused) {
            audio.play();
            btn.innerHTML = `⏸️ Jeda Uji ${index + 1}`;
            playhead.style.display = 'block';
            
            const updateHead = () => {
                if (audio.paused || audio.ended) return;
                const progress = audio.currentTime / audio.duration;
                playhead.style.left = `${progress * cvs.width}px`;
            ...
            audio.pause();
            btn.innerHTML = `▶️ Lanjutkan Uji ${index + 1}`;
            cancelAnimationFrame(state.playbackAnimIds[index]);
        }
    };
    
    audio.onended = () => {
        btn.innerHTML = `▶️ Putar Uji ${index + 1}`;
        cancelAnimationFrame(state.playbackAnimIds[index]);
